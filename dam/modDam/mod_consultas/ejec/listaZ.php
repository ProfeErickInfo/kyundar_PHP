...
include("../../../../enlace/conexion.php");
	...
	$OrderBy = $orden1;
// echo "select * from tbx_deportistas where ".$tpc."  like '$dto%'  order by ".$tpc;
if(($tpc=='id_grado')&&($dto>0)){
	$Query = "select * from tbx_deportistas where ".$tpc."=".$dto." order by ".$OrderBy;
}
//echo "select * from tbx_deportistas where nombres like '%$dto%' or apellidos like '%$dto%' order by ".$OrderBy;
if(($tpc=='nombres')&&($dto!='')){
	$Query = "select * from tbx_deportistas where nombres  like '%$dto%' or apellidos like '%$dto%' order by ".$OrderBy;
}
...
if(($tpc=='localidad')&&($dto!='')){
	...
	$Query = "SELECT * FROM tbx_deportistas as d where d.barrio IN(select l.id from tbx_lista_barrios as l where l.".$tpc."=".$dto.")";

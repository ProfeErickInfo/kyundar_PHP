...
$required_fields = ['txtNombres', 'txtApellidos', 'TipoDocumento', 'txtDocumento', 'txtEmail', 'Sexo', 'txtFechaNac', 'OpSocio'];
$missing_fields = [];
...
// txtCelular es opcional pero lo validamos si viene
if (isset($_POST['txtCelular']) && empty($_POST['txtCelular'])) {
    $missing_fields[] = 'txtCelular';
}
...
$celular = !empty($txtCelular) ? $txtCelular : 1;
$Tpsalud = 1;
$anoHoy = date("Y");
...
if ($CantDep == 0) {
    ...
    $sql_insert = "INSERT INTO trn25_socios (nombres, apellidos, tipo_doc, documento, servsalud, email, nombreEps, ano_lectivo, id_usuario, fecha_edit, fecha_nac, lugar_nac, sexo, barrio, direccion, celular, cod_int, id_club, tipo_socio) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE(), ?, ?, ?, ?, ?, ?, ?, ?, ?)";    $stmt = mysqli_prepare($conexion, $sql_insert);
    ...
    // Bind parameters
    mysqli_stmt_bind_param($stmt, "ssisissiisiissiiii", 
        ...
        $celular,           // celular (i)
        $txtDocumento, // cod_int (i) - documento convertido a entero
        $id_usu,            // id_club (i)

...
function cargarPOST(url, urlreturn, divinfo, divreload, foco) {
	...
	var sData="";
        ...
        iCount = myform.elements.namedItem("idAsA").length;
		eVent=document.getElementById('nEvento').value;
		idGen=document.getElementById('Ngen').value;
		idCat=document.getElementById('Ncat').value;
		idNivel=document.getElementById('nNivel').value;
		mod=document.getElementById('xMod').value;
		nTeam=document.getElementById('txtequipo').value;
		//alert(nTeam);
		idTipo=2;
		esta=1;
		
		if(eVent<1){ alert("Por favor seleccione el evento."); document.getElementById('nEvento').focus(); return; 
			}else xz=xz+1;

		if(idGen<3){ alert("Por favor seleccione el genero mixto."); document.getElementById('Ngen').focus(); return; 
			}else xz=xz+1;
			
		if(idCat<1){ alert("Por favor seleccione la categor�a."); document.getElementById('Ncat').focus(); return; 
			}else xz=xz+1;
		
		if(idNivel<1){ alert("Por favor seleccione el nivel."); document.getElementById('nNivel').focus(); return; 
			}else xz=xz+1;	
			
		if(mod<1){ alert("Por favor seleccione la modalidad."); document.getElementById('xMod').focus(); return; 
			}else xz=xz+1;			
			
		if(nTeam<1){ alert("Por favor escriba el nombre del equipo a registrar."); document.getElementById('txtequipo').focus(); return; 
			}else xz=xz+1;
	
			
			if(xz==6){
				sData +="idAsA=" + eVent + '-' + idGen + '-' + idCat + '-' + idNivel + '-' + mod +  '-' +idTipo + '-' + esta  + '-' + nTeam;
				//alert(sData);
...
/////////////////////////////////////////////////
function cargarSiembra(url,divinfo) {

	var obDiv = document.getElementById(divinfo);
	var siembras = document.getElementsByName('siembra');
	const siembra = [];
	//siembras.push(siembra[0].value);
	//var siembras='';
	for (var i = 0; i <siembras.length; i++) {
		//alert(siembra[i].value);
		siembra.push(siembras[i].value);
...
function enviarFormulario(url, formid, redir, url_redir, loading, divContent, foco){
		...
		const selectedOption = document.querySelector('input[name="OpLugar"]:checked').value;
		const selectedOption2 = document.querySelector('input[name="OpSocio"]:checked').value;
		alert('Lugar: '+selectedOption)
		alert('Socio: '+selectedOption2)
...
function EnviarGrupo(url, divinfo, divreload, foco) {
	
	var obDiv = document.getElementById(divinfo);
	url=url+GetInfoString3();
	mostrar(divreload);
...
function activaChk(){
	//valido 
	//alert(frmbusca1.orden1.value);
	if(frmbusca1.orden1.value=='id_Club')
	{
		frmbusca1.orden1.value='nombres, apellidos';
		return;
	  } else {
	 	frmbusca1.orden1.value='id_Club';
		return;
	}
	//alert(frmbusca1.orden1.value);
...
String.prototype.reverse = function(){
  ...
  function mascaraMoNeda(campo,evento){
...
function SendListaGrados(url, nForm, idcheck, divreload,  urlreturn, divinfo, foco){
	if( ! confirm('Seguro desea asignar el grado seleccionado ?') ){
		return false;
	}
	var nForm=nForm;	
	var gAtleta='';	
	...
	var num=0;
	var ele=0;
	var obj = document.getElementById(nForm.toString()).checklist;
	...
	if( num==0 ){
		alert('Usted no ha seleccionado algún atleta.');
		return false;
	}
	...
	mostrar(divreload);
	cadenaFormulario= 'idgAtleta='+gAtleta+'&num='+num;
...
function SendFormSocio(url, formid, redir, url_redir, loading, divContent, foco){
	...
	for (var i = 0; i < Formulario.elements.length; i++) {
		...
		if (elemento.name) {
			if (elemento.type === 'radio') {
				// Solo incluir radio buttons que estén seleccionados
				if (elemento.checked) {
					cadenaFormulario += sepCampos + elemento.name + '=' + encodeURIComponent(elemento.value);
					sepCampos = "&";
				}
			} else if (elemento.type === 'checkbox') {
				// Solo incluir checkboxes que estén marcados
				if (elemento.checked) {
					cadenaFormulario += sepCampos + elemento.name + '=' + encodeURIComponent(elemento.value);
					sepCampos = "&";
				}
			} else if (elemento.type !== 'button' && elemento.type !== 'submit') {
				// Incluir todos los demás elementos (text, select, hidden, etc.)
				cadenaFormulario += sepCampos + elemento.name + '=' + encodeURIComponent(elemento.value);
				sepCampos = "&";
			}
		}
	...
	console.log('Datos a enviar:', cadenaFormulario);
	
	obXHR.open("POST", url, true);
	obXHR.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
	
	obXHR.onreadystatechange = function () {
		if (obXHR.readyState == 4) {
			ocultar(loading);
			console.log('Respuesta:', obXHR.responseText);
			
			// Verificar si la respuesta es exitosa
			if(obXHR.responseText.indexOf('OK|') === 0) {
				alert('Registro guardado exitosamente');
				Formulario.reset();
				
				if(redir==1){
					// Cargar la página de formulario, no el procesador
					cargarFocus(url_redir,divContent,loading, foco);
				}
			} else {
				// Mostrar el error
...
////////////////////////////////////////////////////////////
// Función simplificada para el formulario r_socio_corto.php
function SendFormSocio_corto(url, formid, redir, url_redir, loading, divContent, foco){
	mostrar(loading);
	...
	// Recorrer solo los campos específicos del formulario corto
	for (var i=0; i <= Formulario.elements.length-1;i++) {
		var elemento = Formulario.elements[i];
		
		if(elemento.name == "OpSocio"){
			// Manejar radio button OpSocio
			if(elemento.checked){
				cadenaFormulario += sepCampos + elemento.name + '=' + encodeURI(elemento.value);
				sepCampos = "&";
			}
		}else if(elemento.name && elemento.name != "btnRegistrar2"){
			// Agregar todos los demás campos excepto el botón
			cadenaFormulario += sepCampos + elemento.name + '=' + encodeURI(elemento.value);
			sepCampos = "&";
		}
	}
	...
	obXHR.onreadystatechange = function () {
		if (obXHR.readyState == 4) {
			...
			// Reset del formulario solo si fue exitoso
			if(obXHR.responseText.includes("OK|")){
				Formulario.reset();
				// Reseleccionar OpSocio por defecto
				document.getElementById('OpSocio1').checked = true;
			}
...
function sendForma(url, formid, redir, url_redir, loading, divContent, foco){
		//alert('aqui js');
		//ocultar('apDivVentana');
		mostrar(loading);
		const selectedOption2 = document.querySelector('input[name="OpSocio"]:checked').value;
